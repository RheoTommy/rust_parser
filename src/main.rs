...
impl Token {
    fn number(n: usize, loc: Loc) -> Self {
...
fn recognize_many(input: &[u8], mut pos: usize, mut f: impl FnMut(u8) -> bool) -> usize {
    while input.len() > pos && f(input[pos]) {
        pos += 1;
    }
    pos
}
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match self {
            ...
            NotOperator(tok) => write!(f, "{}: '{}' is not closed", tok.loc, tok.value),
            UnCloseOpenParen(tok) => write!(f, "{}: '{}' is not closed", tok.loc, tok.value),
...
fn parse_expr2<Tokens>(tokens: &mut Peekable<Tokens>) -> Result<Ast, ParseError>
    ...
    fn parse_expr2_op<Tokens>(tokens: &mut Peekable<Tokens>) -> Result<BinOp, ParseError>
        ...
        let op = tokens
            ...
            .ok_or(ParseError::Eof)
            .and_then(|tok| match tok.value {
                TokenKind::Asterisk => Ok(BinOp::multi(tok.loc.clone())),
...
impl Interpreter {
    ...
    pub fn eval(&mut self, expr: &Ast) -> Result<isize, InterpreterError> {
        use self::AstKind::*;
    ...
    pub fn eval_uni_op(&mut self, op: &UniOp, n: isize) -> isize {
        use UniOpKind::*;
        match op.value {
            Plus => n,
            Minus => -n,
    ...
    pub fn eval_bin_op(&mut self, op: &BinOp, l: isize, r: isize) -> Result<isize, InterpreterErrorKind> {
        use self::BinOpKind::*;
...
impl fmt::Display for InterpreterError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match self.value {
            DivisionByZero => write!(f, "{}: Div by zero", self.loc)
        }
    }
}
